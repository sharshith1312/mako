...
// @unsafe
void txn_logger::Init(size_t nworkers, const vector<string> &logfiles, const vector<vector<unsigned>> &assignments_given, vector<vector<unsigned>> *assignments_used, bool call_fsync, bool use_compression, bool fake_writes) {
  INVARIANT(!g_persist);
  INVARIANT(g_nworkers == 0);
  INVARIANT(nworkers > 0);
  INVARIANT(!logfiles.empty());
  INVARIANT(logfiles.size() <= g_nmax_loggers);
  INVARIANT(!use_compression || g_perthread_buffers > 1);
  vector<int> fds;
  for (auto &fname : logfiles) {
    int fd = open(fname.c_str(), O_CREAT|O_WRONLY|O_TRUNC, 0664);
    if (fd == -1) { perror("open"); ALWAYS_ASSERT(false); }
    fds.push_back(fd);
  }
  g_persist = true; g_call_fsync = call_fsync; g_use_compression = use_compression; g_fake_writes = fake_writes; g_nworkers = nworkers;
  for (size_t i = 0; i < g_nmax_loggers; i++) for (size_t j = 0; j < g_nworkers; j++) per_thread_sync_epochs_[i].epochs_[j].store(0, memory_order_release);
  vector<thread> writers;
  vector<vector<unsigned>> assignments(assignments_given);
  if (assignments.empty()) {
    if (g_nworkers <= fds.size()) { for (size_t i = 0; i < g_nworkers; i++) assignments.push_back({(unsigned) i}); }
    else {
      const size_t threads_per_logger = g_nworkers / fds.size();
      for (size_t i = 0; i < fds.size(); i++) {
        assignments.emplace_back(MakeRange<unsigned>(i * threads_per_logger, ((i + 1) == fds.size()) ? g_nworkers : (i + 1) * threads_per_logger));
      }
    }
  }
  INVARIANT(AssignmentsValid(assignments, fds.size(), g_nworkers));
  for (size_t i = 0; i < assignments.size(); i++) { writers.emplace_back(&txn_logger::writer, i, fds[i], assignments[i]); writers.back().detach(); }
...
void txn_logger::persister(vector<vector<unsigned>> assignments) {
  ...
  for (;;) {
    ...
    if (last_loop_usec < delay_time_usec) {
      ...
      struct timespec t; t.tv_sec = sleep_ns / ONE_SECOND_NS; t.tv_nsec = sleep_ns % ONE_SECOND_NS; nanosleep(&t, nullptr);
    }
    ...
    for (auto j : assignments[i])
      for (size_t k = j; k < NMAXCORES; k += g_nworkers) {
        ...
        if (!ctx.persist_buffers_.peek()) {
          ...
          if (!l.is_locked()) {
            bool did_lock = false;
            for (size_t c = 0; c < 3; c++) { if (l.try_lock()) { did_lock = true; break; } }
            if (did_lock) {
              if (!ctx.persist_buffers_.peek()) {
...
void txn_logger::writer(unsigned id, int fd, vector<unsigned> assignment) {
  ...
  for (;;) {
    ...
    for (auto idx : assignment) {
      ...
      for (size_t k = idx; k < NMAXCORES; k += g_nworkers) {
        ...
        for (auto px : pxs) {
          INVARIANT(px); INVARIANT(!px->io_scheduled_); INVARIANT(nbufswritten <= iovs.size()); INVARIANT(px->header()->nentries_); INVARIANT(px->core_id_ == k);
          if (nbufswritten == iovs.size()) { ++g_evt_logger_writev_limit_met; goto process; }
          if (transaction_proto2_static::EpochId(px->header()->last_tid_) >= cur_sync_epoch_ex + g_max_lag_epochs) { ++g_evt_logger_max_lag_wait; break; }
  ...
  #define PXLEN(px) ((px)->curoff_)
          ...
          if (last_tid_cid != px_cid) { cerr << "header: " << *px->header() << endl; cerr << g_proto_version_str(last_tid_cid) << endl; cerr << "last_tid_cid: " << last_tid_cid << endl; cerr << "px_cid: " << px_cid << endl; }
#endif
          ...
          if (!pes.ntxns_.load(memory_order_acquire)) pes.earliest_start_us_.store(px->earliest_start_us_, memory_order_release);
    ...
    process:
    if (!nbufswritten) { nop_pause(); continue; }
    ...
    if (!g_fake_writes) {
      ...
      if (unlikely(ret == -1)) { perror("writev"); ALWAYS_ASSERT(false); }
      if (g_call_fsync) { const int fret = fdatasync(fd); if (unlikely(fret == -1)) { perror("fdatasync"); ALWAYS_ASSERT(false); } }
#ifdef ENABLE_EVENT_COUNTERS
      {
      ...
      for (size_t k = idx; k < NMAXCORES; k += g_nworkers) {
        ...
        while ((px = ctx.persist_buffers_.peek()) && px->io_scheduled_) {
#ifdef LOGGER_STRIDE_OVER_BUFFER
          {
            ...
            for (size_t p = 0; p < pxlen; p += stridelen) if ((&px->buf_start_[0])[p] & 0xF) non_atomic_fetch_add(ea.dummy_work_, 1UL);
          }
#endif
          px0 = ctx.persist_buffers_.deq(); INVARIANT(px == px0); INVARIANT(px->header()->nentries_); px0->reset(); INVARIANT(ctx.init_); INVARIANT(px0->core_id_ == k); ctx.all_buffers_.enq(px0);
        }
      }
    }
    sense = !sense;
...
tuple<uint64_t, uint64_t, double> txn_logger::compute_ntxns_persisted_statistics() {
  uint64_t acc = 0, acc1 = 0, acc2 = 0, num = 0;
  for (size_t i = 0; i < g_persist_stats.size(); i++) {
  ...
  INVARIANT(acc <= acc1); INVARIANT(acc1 <= acc2);
...
void txn_logger::clear_ntxns_persisted_statistics() {
  for (size_t i = 0; i < g_persist_stats.size(); i++) {
    ...
    for (size_t e = 0; e < g_max_lag_epochs; e++) { auto &pes = ps.d_[e]; pes.ntxns_.store(0, memory_order_release); pes.earliest_start_us_.store(0, memory_order_release); }
  }
}
...
void txn_logger::wait_until_current_point_persisted() {
  ...
  cerr << "waiting for system_sync_epoch_=" << system_sync_epoch_->load(memory_order_acquire) << " to be < e=" << e << endl;
...
void transaction_proto2_static::PurgeThreadOutstandingGCTasks() {
  ...
  for (;;) {
    ...
    if (unlikely(!ro_tick_ex)) { sleep_ro_epoch(); continue; }
    const uint64_t ro_tick_geq = ro_tick_ex - 1;
    if (ro_tick_geq < e) { sleep_ro_epoch(); continue; }
    break;
  }
...
// @unsafe: manual RCU guard construction
...
#define ENTER_RCU() do { new (&rcu_guard[0]) scoped_rcu_base<false>(); } while (0)
#define EXIT_RCU() do { scoped_rcu_base<false> *px = (scoped_rcu_base<false> *) &rcu_guard[0]; px->~scoped_rcu_base<false>(); } while (0)
  ...
  // Move iterator outside loop to fix "variable not alive" borrow checker error
  auto it = q.begin();
  auto end = q.end();
  for (; it != end; ++it, ++n, ++niters_with_rcu) {
    ...
    if (!delent.key_.get_flags()) {
      ...
      if (unlikely(!delent.tuple()->is_latest())) {
        ...
        ctx.queue_.enqueue(delete_entry(nullptr, MakeTid(CoreMask, NumIdMask >> NumIdShift, (my_ro_tick + 1) * ReadOnlyEpochMultiplier - 1), delent.tuple(), marked_ptr<string>(), nullptr), my_ro_tick);
      ...
      if (!in_rcu) { ENTER_RCU(); niters_with_rcu = 0; in_rcu = true; }
    ...
    if (in_rcu && niters_with_rcu >= max_niters_with_rcu) { EXIT_RCU(); niters_with_rcu = 0; in_rcu = false; }
  }
  q.clear();
